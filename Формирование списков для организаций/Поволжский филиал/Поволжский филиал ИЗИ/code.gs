...
function check(){
  ...
  if(cell.indexOf("P">=0) && value === "Оформлен")
  {
    ...
    for(let i = 0; i < mainUuuidsRange.length; i++)
    {
      ...
      {
        main.getRange(i+2,17).setValue("Оформлен");
        return;
      }
    }
  }
...
function checkV2(){
  ...
  if(cell.indexOf("M">=0))
  {
    ...
    let main = SpreadsheetApp.openById("1xJ6jn-ecUOj6SQ8DlxW34AYxWYGyaSTeE6hh5UitFWQ").getSheetByName("Оформление_V.2");
...
function checkByUUIDV2(){
  ...
  let mainSheet = SpreadsheetApp.openById("1xJ6jn-ecUOj6SQ8DlxW34AYxWYGyaSTeE6hh5UitFWQ").getSheetByName("Оформление_V.2");
