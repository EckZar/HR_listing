...
function transferIsi(){
  ...
  let newSheet = SpreadsheetApp.openById("1Yo6E03M1PHuZDrr4rnQgvHLamYbUJOkpxJMjFNJcbtU").getSheetByName("Оформление_V.2");
  ...
  range = range.filter(function(e){ return e[14] != "" && e[11].indexOf("ИЗИ")>=0; })
               ...
               .forEach(function(item){newSheet.getRange(getPos(item[1], "1Yo6E03M1PHuZDrr4rnQgvHLamYbUJOkpxJMjFNJcbtU", "Оформление_V.2"), 1, 1, 15).setValues([item]); })
}
...
//=========================================================================
function checks(){
  check("1Yo6E03M1PHuZDrr4rnQgvHLamYbUJOkpxJMjFNJcbtU"); // ИЗИ
...
function syncs(){
  ...
  sync("1Yo6E03M1PHuZDrr4rnQgvHLamYbUJOkpxJMjFNJcbtU"); // ИЗИ
...
function checkStatus(){
  ...
  {
    Logger.log(value);
    let range = cell.slice(1, cell.length);
    Logger.log(range);
    let vals = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Оформление_V.2").getRange(range, 12, 1, 4).getValues(); 
    let main = vals[0][0].indexOf("ИЗИ")>=0 ? isiSheet : commonLkSheet;
    Logger.log(vals);
    Logger.log(main.getSheetId());
    let mainUuuidsRange = main.getRange(2, 15, main.getLastRow()-1, 1).getValues();
    
    for(let i = 0; i < mainUuuidsRange.length; i++)
    {
      if(mainUuuidsRange[i][0] === vals[0][3])
      {
